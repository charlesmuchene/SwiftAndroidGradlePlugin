...
func mapPixelToComplex(
    ...
    // 1. Convert pixel coordinates to a normalized value (0.0 to 1.0)
    ...
    // 2. Map normalized values to the complex number range (Real Part)
    // Real value = xMin + normX * (xMax - xMin)
    let cReal = xMin + normX * (xMax - xMin)
    
    // 3. Map normalized values to the complex number range (Imaginary Part)
...
func generateMandelbrotGrid(
    ...
    for y in 0..<height {
        for x in 0..<width {
            ...
            // 2. Calculate the escape count for that complex number
            let count = mandelbrotEscapeCount(c: c, maxIterations: maxIterations)
            
            // 3. Store the result in the grid at the corresponding (y, x) position
            grid[y][x] = Int(count)
        }
    }
...
/// 1. Strategy for classic, banded coloring.
...
/// 2. Strategy for smooth, continuous coloring.
...
/// 3. Strategy for coloring the interior of the set differently.
...
func renderMandelbrot(
    ...
    yMin: Double,
    yMax: Double,
    strategy: MandelbrotColoringStrategy // Accepts any strategy!
) -> [[Double]] {
    ...
    for y in 0..<height {
        for x in 0..<width {
            ...
            // 1. Get the raw (smoothed) escape count
            let rawCount = mandelbrotEscapeCount(c: c, maxIterations: maxIterations)
            
            // 2. Pass the count to the chosen strategy to get the final color index
